    ...
    <div class="card">
      <div class="card-body">
        ...
        <form class="forms-sample" method="POST" action="{{ route('store.subcategory') }}">
          ...
          <div class="form-group">
            <label for="exampleInputEmail1">Category Japanese</label>
            <input type="text" class="form-control" name="subcategory_jp"  placeholder="SubCategory Japanese">
            @error('subcategory_jp')
            <span class="text-danger">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="category_id">
                <option disabled="" selected="">--Select Category</option>
                    ...
                    <option value="{{ $row->id }}">{{ $row->category_en }} | {{ $row->category_jp }}
                </option>
                    @endforeach
            </select>
          </div>
